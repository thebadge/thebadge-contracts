...
import { IArbitrable } from "../../../lib/erc-792/contracts/IArbitrable.sol";
import { IEvidence } from "../../../lib/erc-792/contracts/erc-1497/IEvidence.sol";
import "../../utils/CappedMath.sol";
import "../../utils/CappedMath128.sol";
...
contract LightGeneralizedTCR is IArbitrable, IEvidence {
    ...
    function addItem(string calldata _item) external payable {
        ...
        require(item.requestCount < type(uint120).max, "Too many requests for item.");
        require(item.status == Status.Absent, "Item must be absent to be added.");
        ...
        if (msg.value > totalCost) {
            requester.transfer(msg.value - totalCost); // Use transfer to ensure transaction reverts on failure
        }
    }
    ...
    function executeRequest(bytes32 _itemID) external {
        ...
        if (sumDeposit > 0) {
            ...
            request.requester.transfer(sumDeposit);
        }
    }
    ...
    function rule(uint256 _disputeID, uint256 _ruling) external {
        ...
        if (winner == Party.None) {
            ...
            request.requester.transfer(halfSumDeposit);
            request.challenger.transfer(halfSumDeposit);
            ...
            request.challenger.transfer(sumDeposit);
        }
